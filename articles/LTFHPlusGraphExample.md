...
```
    ...
    ## IGRAPH eec4070 DN-- 31 44 -- 
    ## + attr: name (v/c), lower (v/n), upper (v/n)
    ## + edges from eec4070 (vertex names):
...
including individuals that are within $n$ steps of said proband. The $n$
steps correspond to $n$ degree relatives, such that $n = 1$ yields all
first degree relatives, $n = 2$ yields all second degree relatives,
etc.. From this local graph, we can construct a kinship matrix and the
covariance matrix required by
[`estimate_liability()`](https://emilmip.github.io/LTFHPlus/reference/estimate_liability.md).
...
present for a given proband up to degree $2$ (can be generalised to any
integer $n$), which allows us to construct the kinship matrix. Next, we
have attached the lower and upper threshold for each individual during
the construction of the graph, and as such follow the indivdual in
whatever local graph they are a part of. Finally, we need a
