...
requires identifying parents (role for parents are $m$ and $f$), then
one can identify siblings (roles for siblings are $s1,s2,etc.$), and so
on for all available roles. The available roles can be seen in the
documentation for
[`estimate_liability()`](https://emilmip.github.io/LTFHPlus/reference/estimate_liability.md).
