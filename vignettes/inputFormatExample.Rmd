...
In the above example, <tt>fam_ID</tt> from <tt>sims</tt> correspond to the family input, with the first entry, <tt>indiv_ID</tt> containing the ID for the proband, and <tt>fam_id</tt> contains list entries with the family member's ID and familial relationship.
...
# only with parents
construct_covmat(fam_vec = c("m", "f"), sq.herit = .5)
construct_covmat(fam_vec = NULL, n_fam = setNames(c(1,1), c("m", "f")))

# no family members
construct_covmat(fam_vec = NULL, n_fam = NULL, sq.herit = .5)
```
