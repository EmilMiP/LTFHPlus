...
estimate_gen_liability_ltfh = function(h2,
  ...
  reduced = phen %>% 
    select(!!as.symbol(status_col_offspring),!!as.symbol(status_col_father), !!as.symbol(status_col_mother), !!as.symbol(status_col_siblings)) %>% 
    group_by(!!as.symbol(status_col_offspring),!!as.symbol(status_col_father), !!as.symbol(status_col_mother), !!as.symbol(status_col_siblings)) %>% 
    slice_head() #extract each unique configuration present in the data.
